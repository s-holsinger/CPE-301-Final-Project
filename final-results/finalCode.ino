/*
This is the final code and the code you need to upload to make the device function.
Let us know is we need to demonstrate the device in person.
(This is Sebastian, I have all the neccesary hardware, and I am one making the offer. Reach out to me first if you must.)

Be aware that the sensor is currently configured to detect breaths from ~4 inches away
(you can change this with the breathThreshold variable, which is the sensor value where a breath is detected).

The best source to learn how to use the device is the video.
The code is messy, and we are sorry if it take a while to analyse.
However, it should be functional.
Almost everything is in GPIO.
However, we didn't have time to write a replacement for the delay() function--it is used in getSensorData().
There was also a pin binding under setup() that was being problematic; please view the comment.
